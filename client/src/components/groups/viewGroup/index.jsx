...
import { MyBalance } from "../../dashboard/MyBalance";
...
export default function ViewGroup() {
  ...
  const toggleExpView = () => setViewSettlement(0);
  const toggleSettleView = () => setViewSettlement(1);
  const toggleMySettleView = () => setViewSettlement(2);

  const mdUp = useResponsive("up", "md");

  const categorizeTransaction = (expense) => {
    if (expense?.expenseAmount > 0) {
      return { type: "Owed", amount: expense.expenseAmount };
    } else if (expense?.expenseAmount < 0) {
      return { type: "Owe", amount: Math.abs(expense.expenseAmount) };
    }
  ...
  useEffect(() => {
    const getGroupDetails = async () => {
      ...
      if (response_group) setGroup(response_group?.data?.group);
      if (response_expense) {
        setGroupExpense(response_expense?.data);
        setExpenses(response_expense?.data?.expense?.slice(0, 5) || []);
      }
      if (
        !response_expense?.data?.expense ||
        response_expense?.data?.expense?.length <= 5
      ) {
        setShowAllExp(true);
      }
      setLoading(false);
          ...
          >
            ...
            <Box
              sx={{
                ...
                WebkitMask:
                  "url(/static/icons/shape-avatar.svg) no-repeat center / contain",
                zIndex: 9,
                          ...
                          <Grid item xs={12} md={expFocus ? 12 : 6}>
                            <Grid container spacing={2}>
                              {expenses?.map((myExpense) => (
                                <Grid item xs={12} key={myExpense?._id}>
                                  <ExpenseCard
