...
exports.viewUserExpense = async (req, res) => {
  try {
    ...
    const userExpense = await model.Expense.find({
    ...
    if (userExpense.length === 0) {
      const err = new Error("No expense present for the user");
      err.status = 400;
      throw err;
    }
    ...
    for (const expense of userExpense) {
      ...
      if (expense.expenseOwner === userEmail) {
        // You paid — others owe you
        const othersOwe = expense.expensePerMember * (expense.expenseMembers.length - 1);
        youAreOwed += othersOwe;
      } else {
        // You owe the owner
        youOwe += expense.expensePerMember;
      }
    }
