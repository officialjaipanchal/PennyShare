...
//         id: user._id.toString(), // Ensure user ID is stored in the token
...
exports.userLogin = async (req, res) => {
  try {
    // Checking email Id exists in DB
    ...
    console.log("User found in DB:", user); // Debugging statement
    ...
    const accessToken = apiAuth.generateAccessToken(user);

    res.status(200).json({
      status: "Success",
      message: "User Login Success",
      userId: user._id, // Ensure this matches the generated token
      emailId: user.emailId,
      firstName: user.firstName,
      lastName: user.lastName,
      accessToken,
